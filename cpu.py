class CPU:
    ...
    def __init__(self, usertime, nicetime, systime, idletime, iotime, irqtime, softirqtime, stealtime):
        self.usertime = usertime # user   (1) Time spent in user mode.
        self.nicetime = nicetime # nice   (2) Time spent in user mode with low priority (nice).
        self.systime = systime # system (3) Time spent in system mode.
        self.idletime = idletime # idle   (4) Time spent in the idle task.  This value should be USER_HZ times the second entry in the /proc/uptime pseudo-file.
        self.iotime = iotime # iowait (since Linux 2.5.41) (5) Time waiting for I/O to complete.
        self.irqtime = irqtime # irq (since Linux 2.6.0-test4) (6) Time servicing interrupts.
        self.softirqtime = softirqtime # softirq (since Linux 2.6.0-test4) (7) Time servicing softirqs.
        self.stealtime = stealtime # steal (since Linux 2.6.11) (8) Stolen time, which is the time spent in other operating systems when running in a virtualized environment
        self.totaltime = self.usertime + self.nicetime + self.systime + self.idletime + self.iotime + self.irqtime + self.softirqtime + self.softirqtime + self.stealtime
